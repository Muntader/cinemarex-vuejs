...
const module = {
    ...
    actions: {
        ...
        GET_ALL_COLLECTION({commit}) {
            ...
            axios.get( 'http://localhost:8000/api/v1/get/collection').then((response) => {
        ...
        GET_COLLECTION({ commit }, id) {
            commit('COLLECTION_SPINER_LOAD');
            axios.get( 'http://localhost:8000/api/v1/get/collection/' + id ).then((response) => {
                if (response.status === 200) {
        ...
        ADD_NEW_TO_COLLECTION({ commit }, { id, new_collection, already_collection, type}) {
            commit('COLLECTION_SPINER_LOAD');
            axios.post( 'http://localhost:8000/api/v1/create/item/collection', { id, new_collection, already_collection, type }).then((response) => {
                if (response.status === 200) {
                    alertify.logPosition('bottom center');
        ...
        EDIT_COLLECTION({ commit }, { id, name}) {
            axios.post( 'http://localhost:8000/api/v1/update/collection', { id, name }).then((response) => {
                if (response.status === 200) {
                    alertify.logPosition('bottom center');
                    alertify.success(response.data.message);
                }
        ...
        DELETE_COLLECTION({ commit }, id) {
            axios.post( 'http://localhost:8000/api/v1/delete/collection', {id: id}).then((response) => {
                if (response.status === 200) {
                    alertify.logPosition('bottom center');
                    alertify.success(response.data.message);
        ...
        DELETE_FROM_COLLECTION({ commit }, { id, type }) {
            commit('COLLECTION_SPINER_LOAD');
            axios.post( 'http://localhost:8000/api/v1/delete/item/collection', { id, type }).then((response) => {
                if (response.status === 200) {
                    alertify.logPosition('bottom center');
                    alertify.success(response.data.message);
                    commit('COLLECTION_SPINER_CLEAN');
        ...
        ADD_LIKE({ commit }, { id, type }) {
            axios.post( 'http://localhost:8000/api/v1/create/like', { id, type }).then((response) => {
                if (response.status === 200) {
